...
def load_encoder_checkpoint_from_pretext_model(path_to_checkpoint: str) -> ResNetEncoder:
    checkpoint = torch.load(path_to_checkpoint)
    if "simclr" in path_to_checkpoint.lower():
        return AddProjection(config=get_simclr_config()).load_state_dict(checkpoint).backbone
        ...
        raise ValueError(f"Checkpoint name has to contain simclr, vae, or gan but was {path_to_checkpoint}")
