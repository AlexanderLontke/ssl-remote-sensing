...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(config.d_featuremaps, config.d_featuremaps * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(config.d_featuremaps * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (config.d_featuremaps*2) x 16 x 16
            nn.Conv2d(config.d_featuremaps * 2, config.d_featuremaps * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(config.d_featuremaps * 4),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (config.d_featuremaps*4) x 8 x 8
            nn.Conv2d(config.d_featuremaps * 4, config.d_featuremaps * 8, 4, 2, 1, bias=False),
